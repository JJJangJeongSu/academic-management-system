...
import { createNotice } from '../api/course';
...
const CreateNoticeForm: React.FC<CreateNoticeFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createNotice(courseId, postName, postString, appendix || undefined);
      onSuccess();
    } catch (err) {
      setError(err instanceof Error ? err.message : '공지사항 작성에 실패했습니다.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 space-y-4">
      <div className="flex items-center justify-between border-b pb-4">
        <h3 className="text-lg font-semibold text-secondary-900">공지사항 작성</h3>
        <button
          type="button"
          onClick={onCancel}
